...
# Lấy token từ Secrets (Khuyên dùng) hoặc dùng trực tiếp
...
def get_online_templates():
    """Tự động quét danh sách mẫu từ GitHub"""
...
def upload_to_github(file_name, file_bytes):
    """Bà xã upload file trực tiếp lên kho lưu trữ GitHub"""
    ...
    # Check nếu file đã tồn tại để lấy SHA
    check_res = requests.get(url, headers=headers)
    if check_res.status_code == 200:
        data["sha"] = check_res.json()["sha"]

    res = requests.put(url, headers=headers, json=data)
    return res.status_code in [200, 201], res.json().get('message', '')

# --- LOGIC XỬ LÝ WORD (Tối ưu thứ tự biến) ---
def get_variables_v10(file_stream):
    try:
...
# --- GIAO DIỆN CHÍNH ---
if login():
    st.set_page_config(page_title="Bank Form Auto", layout="wide")
    
    # CSS tạo hiệu ứng tờ giấy A4 chuẩn Sacombank
    st.markdown("""
        <style>
    ...
    if file_bytes:
        vars_list = get_variables_v10(io.BytesIO(file_bytes.getvalue()))
        tab_in, tab_pre = st.tabs(["📝 NHẬP LIỆU", "🔍 XEM TRƯỚC & XUẤT FILE"])
        ...
        with tab_in:
            for item in vars_list:
                ...
                if item['type'] == 'title':
                    ...
                    with c1:
                        v = st.session_state.form_data.get(k, False if item['type'] == 'checkbox' else "")
        ...
        with tab_pre:
            ...
            try:
                ...
                st.divider()
                st.download_button("🚀 XUẤT FILE WORD HOÀN TẤT", data=out.getvalue(), file_name=f"Done_{selected}")
