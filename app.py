...
# Lấy token từ Secrets (Khuyên dùng) hoặc dùng trực tiếp từ fen cung cấp
# Token: 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
...
def upload_to_github(file_name, file_bytes):
    ...
    data = {"message": f"Bà xã cập nhật mẫu: {file_name}", "content": encoded_content, "branch": "main"}
    
    # Lấy SHA nếu file đã tồn tại để thực hiện ghi đè
    ...
    return False, f"Lỗi: {res.json().get('message', 'Không xác định')}"

# --- LOGIC XỬ LÝ BIẾN WORD ---
def get_variables_v11(file_stream):
    try:
...
def login():
    ...
    if st.button("Đăng nhập"):
        if pwd == "phi1992": # Mật khẩu dành cho bà xã
...
if login():
    ...
    st.markdown("""
        <style>
        .cat-box { background: #f8f9fa; padding: 12px; border-left: 6px solid #d32f2f; margin: 20px 0; }
        .paper-view { background: #525659; padding: 30px 0; display: flex; justify-content: center; }
        .a4-page { background: white; width: 210mm; min-height: 297mm; padding: 20mm; box-shadow: 0 0 15px rgba(0,0,0,0.5); font-family: 'Times New Roman', serif; }
    ...
    with st.sidebar:
        ...
        if st.button("🗑️ Xóa sạch Form"):
            st.session_state.form_data = {}; st.rerun()

    file_bytes = None
    if active_url: file_bytes = io.BytesIO(requests.get(active_url).content)

    if file_bytes:
        vars_list = get_variables_v11(io.BytesIO(file_bytes.getvalue()))
        ...
        with tab_in:
            ...
            for item in vars_list:
                ...
                if item['type'] == 'title':
                    ...
                    with c1:
                        # Tự động điền ngày tháng năm hiện tại nếu rỗng
                        default_val = ""
                        if "ngay" in item['label'].lower(): default_val = str(now.day)
                        elif "thang" in item['label'].lower(): default_val = str(now.month)
                        elif "nam" in item['label'].lower(): default_val = str(now.year)
                        
                        v = st.session_state.form_data.get(k, False if item['type'] == 'checkbox' else default_val)
